...
(function () {
	...
	var gmeResources = {
			parameters: {
				// Defaults
				version: "0.9.4.br",
				versionMsg: "Honors GPX track color",
				...
				<div class="gme-tab-content">\
					<div class="gme-fieldgroup">\
						...
						<ul>\
							...
							<li><label title="Display dropped down track with color, opacity and weight specified in the GPX"><input type="checkbox" name="GME_trkColor" id="GME_trkColor" /> Track colors</label></li>\
			...
			script: {
				...
				config: function () {
					...
					function setConfig() {
						...
						$("#GME_trkColor").attr("checked", that.parameters.trackColors);
					...
					function storeSettings() {
						...
						that.parameters.trackColors = $("#GME_trkColor")[0].checked ? true : false;
					...
					var dropHandlerObj = {
						...
						drop: function (map) {
							return function (e) {
								...
								function buildPolylineOptions(col, opa, wei) {
									if (!!col || !!opa || !!wei) {
										var opts = { };
										if (!!col) opts.color = /^#?[0-9a-f]{6,}$/i.test(col) ? '#' + col.substring(col.length - 6) : col;
										opts.opacity = opa ?? "1.0";
										if (!!wei) opts.weight = wei;
										debugger;
										return opts;
									}
									return null;
								}
								function garminRouteExtensionToPolylineOptions(element) {
									return buildPolylineOptions(
										element.find("extensions > gpxx\\:RouteExtension > gpxx\\:DisplayColor")?.text(),
										null,
										null
									);
								}
								function topografixLineExtToPolylineOptions(element) {
									return buildPolylineOptions(
										element.find("extensions > line > color")?.text(),
										element.find("extensions > line > opacity")?.text(),
										element.find("extensions > line > width")?.text()
									);
								}
								function locusLineExt2ToPolylineOptions(element) {
									return buildPolylineOptions(
										element.find("extensions > line > extensions > locus\\:lsColorBase")?.text(),
										null,
										element.find("extensions > line > extensions > locus\\:lsWidth")?.text()
									);
								}
								function parsePolylineOptions(element) {
									if (!!that.parameters.trackColors) {
										var opts = locusLineExt2ToPolylineOptions(element);
										if (!opts) {
											opts = topografixLineExtToPolylineOptions(element);
											if (!opts) {
												opts = garminRouteExtensionToPolylineOptions(element);
											}
										}
										return opts;
									}
								...
								function parseGPX(text) {
									var d, i, j, k, w, r, t, lat, lng, len, n, opts, name = "", poly, type, points = { primary: [], additional: [], routes: [] }, gpx = $($.parseXML(text)), wpts = gpx.find("wpt"), tracks = gpx.find("trk"), segs, routes = gpx.find("rte");
									for (i = 0, len = wpts.length; i < len; i++) {
										w = $(wpts[i]);
										lat = w.attr("lat");
										lng = w.attr("lon");
									...
									for (i = routes.length - 1; i >= 0; i--) {
										...
										points.routes.push({ name: name, points: poly, opts: opts ?? { } });
									}
									for (i = tracks.length - 1; i >= 0; i--) {
										...
										for (j = segs.length - 1; j >= 0; j--) {
											var opt2 = parsePolylineOptions($(segs[j]));
											n = r.filterNode("name");
											...
											points.routes.push({ name: name, points: poly, opts: opt2 ?? opts ?? { } });
										}
									}
						...
						if (plist.routes) {
							for (i = plist.routes.length - 1; i >= 0; i--) {
								if (plist.routes[i].points && plist.routes[i].points.length > 0) {
									layers.addLayer(L.polyline(plist.routes[i].points, plist.routes[i].opts ? plist.routes[i].opts : { }));
								}
							}
						}
